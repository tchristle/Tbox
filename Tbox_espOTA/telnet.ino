  ...
  for(i = 0; i < MAX_SRV_CLIENTS; i++){
    if (serverClients[i] && serverClients[i].connected()){
      if(serverClients[i].available()){
        ...
        cmd_msg = espmsg;
        Serial.print(i);Serial.print(":");Serial.println(cmd_msg);
        cmd_msg.trim();
        command_lookup(cmd_msg);
        Serial.println();
        Serial.print("Ready:>");
        //Serial.println(cmd_msg.substring(0,3));
        //if(cmd_msg.substring(0,3) == "rot") do_cmove();
        //else if(cmd_msg.substring(0,3) == "scn") wifi_scan();
        //else do_move();
      }
    }
  }
...
void sp(String msg){
  int i;
  //String telmsg = rcvstr.substring(2); 
  //Serial.print('\n');
