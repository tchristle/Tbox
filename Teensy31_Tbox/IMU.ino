...
void magdata()
{
  ...
  while(MySerial.available()<1)
    {
      imu.readMag();
      calx = ((imu.mx - mxoffset)*mxscale);
      caly = ((imu.my - myoffset)*myscale);
      
      MySerial.print(calx); MySerial.print(" "); MySerial.print(caly); MySerial.print(" "); 
      MySerial.println(calcHeading(calx, -caly));
...
void madgwick(){
  ...
  while(MySerial.available()<1){
    ...
    imu.readGyro();
    Gx = -1 * imu.calcGyro(imu.gx-gxbias);
    Gy = -1 * imu.calcGyro(imu.gy-gybias);
    Gz = -1 * imu.calcGyro(imu.gz-gzbias);
    ...
    ESP_Ser.print(Yaw); ESP_Ser.print(" "); 
    ESP_Ser.print(Pitch); ESP_Ser.print(" "); 
    ESP_Ser.println(Roll); //MySerial.print(" "); 
    //MySerial.print(time1); 
    //MySerial.println();

    //ESP_Ser.println(360-Yaw);
  }
